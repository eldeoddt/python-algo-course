# 문제 1 배수 더하기
# 1~100 정수 중 4 배수이면서 6의 배수는 아닌 수의 합을 구하시오.
...
# 소수 판별
# 1과 자기 자신으로만 나눠지면 소수이다.
# 모든 수 돌려서 나눠지는수가 잇으면 소수가 아닌것으로 판단한다.
...
# print("그때까지의 합 ", sum)
# 꼭 변수를 증가시켜줘야 하고, break도 넣어줘야 한다.
# sum<=1000 조건을 while에 넣었더니 마지막에 num 증가부분 때문에 num이 하나 더 많게 나왔다.
# while 탈출 시에는 if로 명시적으로 하자
...
while (True):
    mid = int((low + high) / 2)
    select = int(input(f"당신이 입력한 숫자는 {mid}입니다. 1.OK, 2.UP, 3.DOWN "))
    if (select == 1):
        break
    elif (select == 2):
        low = mid + 1
...
# 그런데 예시의 출력값과 값이 하나씩 자꾸 다르게 나왔다.
# 문제는 mid값을 그대로 옮기는 것에 있었다. +1, -1을 해서 mid값은 포함 안하도록 해야 한다.
# 그래도 {mid:.0f}로 하면 자동 반올림 돼서 예시 출력과 다르게 나왔다.
# -> mid값에 int()를 적용해서 버림 하여 해결했다.
